...
function calculateStopTicks(depth, o2Pct, majoration = 0) {
    if (state.isGFMode) {
        if (!window.dataManager.getGFTicks) return [];
        const gfTicks = window.dataManager.getGFTicks(state.currentGFLow, state.currentGFHigh, o2Pct, depth);
        
        return gfTicks.sort((a,b) => a.minDuration - b.minDuration).map(t => ({
            value: t.minDuration,
            label: `${t.stopDepth}m`,
