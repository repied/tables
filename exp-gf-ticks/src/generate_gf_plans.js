...
////////////// PARAMETERS //////////////
const num_samples = 50_000; // Total number of random samples to generate
////////////// PARAMETERS //////////////
...
const total = num_samples;
const logInterval = Math.max(1, Math.floor(total / 100)); // log ~100 times
let count = 0;
const startTime = Date.now();

console.log(`Generating ${total} random dive plans...`);
for (let i = 0; i < total; i++) {
    const gfLow = randomInRange(MIN_GF, MAX_GF);
    const gfHigh = randomInRange(MIN_GF, MAX_GF);
    const o2 = randomInRange(MIN_O2, MAX_O2);
    const depth = randomInRange(MIN_DEPTH, MAX_DEPTH);
    const duration = randomInRange(MIN_TIME, MAX_TIME);
