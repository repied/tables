...
// Expectation from table (verified in CSV): Group I, Stop 3m: 4min. DTR 6.
{
    ...
    // Ascent 3->0 = 0.5
    // Total = 5.633 -> ceil -> 6
    assertEqual(dtr, 7, "DTR should be 7 min");
}
...

{
    ...
    check_dtr_single_dive(20, 50, 7);
    check_dtr_single_dive(20, 75, 27);
    check_dtr_single_dive(20, 90, 37);
    check_dtr_single_dive(10, 75, 1);
    check_dtr_single_dive(10, 360, 3);
    check_dtr_single_dive(34, 20, 9);
    check_dtr_single_dive(34, 45, 52);
    check_dtr_single_dive(55, 5, 7);
    check_dtr_single_dive(55, 50, 138); // diveapp gives 134 but maybe different ascent speed
...

{
    ...
    check_successive_dive('G', 180, 40, 7);
...
// Test 10: Advanced Gas Consumption
{
    ...
    if (Math.abs(gasNoStops - 666) > 2) {
        console.error(`❌ Simple ascent gas failed. Expected ~666, got ${gasNoStops}`);
        failed++;
    } else {
    ...
    if (Math.abs(gasWithStops - 1854) > 5) {
        console.error(`❌ Gas with stops failed. Expected ~1854, got ${gasWithStops}`);
        failed++;
    } else {
